...
void BinarySearchTree::find(std::string val) {
    bstNode* node = findBst(this->root,val);
    if(node->key == val)
        std::cout<<"1"<<std::endl;
    else
        std::cout<<"0"<<std::endl;
...
void BinarySearchTree::successor(std::string val) {
    bstNode* x = findBst(this->root,val);
    if(x == this->NIL)
        std::cout<<std::endl;
    else
...
void BinarySearchTree::deleteBst(std::string val){
    bstNode* node = findBst(this->root,val);
    if(node != this->NIL)
        this->size--;
    if(node->left == this->NIL) {
        transplantBst(node, node->right);
    } else if(node->right == this->NIL) {
        transplantBst(node, node->left);
    } else {
        bstNode* y = minBst(node->right);
        if(y->p != node){
            transplantBst(y,y->right);
            y->right = node->right;
            y->right->p = y;
        }
        transplantBst(node,y);
        y->left = node->left;
        y->left->p = y;
    }
}
...
bstNode* BinarySearchTree::findBst(bstNode* x, std::string val){
